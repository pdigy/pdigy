...
class pdigyDecoder:
    ...
    def reconstruct_image_from_patches(self, patches, patch_size, original_image_size):
        ...
        reconstructed_image = np.zeros((original_image_size[0], original_image_size[1], 3), dtype=np.uint8)
    ...
    def decode_file(self):
        with open(self.file_path, 'rb') as file:
            ...
            if file_lead != expected_lead or file_version != expected_version or file_name != expected_name:
            ...
            serialized_patches_size = self._hex_to_size(serialized_patches_size_bytes)
            compressed_patches_data = file.read(serialized_patches_size)
            serialized_patches_data = zipper.decompress(compressed_patches_data)
            patches = pickle.loads(serialized_patches_data)

            thumbnail_image = Image.open(io.BytesIO(thumbnail_data))

            full_image = self.reconstruct_image(patches, n_patches_x, n_patches_y)
    ...
    def reconstruct_image(self, patches, dimx, dimy):
        ...
        full_image = Image.new('RGB', (patch_width * dimx, patch_height * dimy))

        # Stitch the patches
        for i in range(dimy):
            for j in range(dimx):
                patch_image = Image.open(io.BytesIO(patches[i * dimx + j]))
                full_image.paste(patch_image, (j * patch_width, i * patch_height))
